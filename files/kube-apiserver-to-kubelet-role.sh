...
kubectl create clusterrolebinding magnum-user-priv --clusterrole=cluster-admin --user="Magnum User"
kubectl create configmap -n kube-system --from-file /etc/kubernetes/kube_openstack_config cloud-config
...
kubectl apply --validate=false -f /etc/kubernetes/manifests/cinder-csi-plugin
